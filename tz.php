...
<style type="text/css">
<!--
body{text-align: center; margin: 0 auto; padding: 0; background-color:#FFFFFF;font-size:12px;font-family: Tahoma, "Microsoft Yahei", Arial;}
...
<div id="page">
    <div id="header">
        <h1><a href="t.php"><?php echo $title;?></a></h1>
        <div id="download"><A HREF="http://www.yahei.net/tz/tz.zip">探针下载</A></div>
    </div>
...
<table width="100%" cellpadding="3" cellspacing="0" align="center">
  ...
  <tr>
    <td>CPU使用状况</td>
    <td colspan="5"><?php if('/'==DIRECTORY_SEPARATOR){echo $cpu_show." | <a href='".$phpSelf."?act=cpu_percentage' target='_blank' class='static'>查看图表</a>";}else{echo "暂时只支持Linux系统";}?>&nbsp;&nbsp;<a href="t.php">刷新</a>&nbsp;&nbsp;<i><a href='top.php' target='_blank'>Top</a></i>
	</td>
  </tr>
    <tr>
    <td>硬盘使用状况</td>
...
<?php preg_match_all( "/([^\s]+):[\s]{0,}(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)/", $strs[$i], $info );?>
     <tr>
        ...
        <td width="29%">已接收: <font color='#CC0000'><span id="NetInput<?php echo $i?>"><?php echo $NetInput[$i]?></span></font></td>
		<td width="14%">实时: <font color='#CC0000'><span id="NetInputSpeed<?php echo $i?>">0B/s</span></font></td>
        <td width="29%">已发送: <font color='#CC0000'><span id="NetOut<?php echo $i?>"><?php echo $NetOut[$i]?></span></font></td>
