	...
	this.Show = function(id){
		...
		if(id != CurrentId){
			...
			if(guest_win_mode == "multi"){
				...
				$('#win' + CurrentId).css('visibility', 'hidden').find('.x-user').removeClass("x-now");
			}
...
function guest_create(gid, n, lang, old, au, mb, iz, fr){
	...
	if(i > -1){ //表示客人重新上线
		...
		g_online.prepend('<div id="g' + gid + '" onclick="openWin(' + gid + ');" class="g" title="' + n + ' (' + iz + ')"><div>' + ((mb == 1)? '<u></u>'  : '') + '<b>0</b></div><s>' + iz + '</s><i>' + n + '</i><a onclick="shutGuest(event, ' + gid + ')">X</a><p></p></div>');
