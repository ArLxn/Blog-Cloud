...
class c_rating extends Admin{
	...
	public function updaterating(){
		...
		Success('rating?p=' . $page. FormatUrlParam(array('s'=>urlencode($search), 'g'=>$groupid, 't'=>$time, 'o'=>$order)));
	}
	...
	public function index(){
		...
		TableRow(array('<a class="do-sort" for="rid">ID</a>', '<a class="do-sort" for="gid">客人ID</a>', '<a class="do-sort" for="aid">被评价客服(ID)</a>', '<a class="do-sort" for="score">星星数</a>', '意见建议', '<a class="do-sort" for="time">评价时间</a>', '<input type="checkbox" id="checkAll" for="deleterids[]"> <label for="checkAll">删除</label>'), 'tr0');
		...
		if($maxrows['value'] < 1){
			...
			while($msg = APP::$DB->fetch($getrating)){
				...
				Iif(isset($admins[$msg['aid']]), "<a title=\"编辑客服\" href=\"" . BURL('users/edit?aid='.$msg['aid']) . "\">" . $admins[$msg['aid']] . "($msg[aid])</a>", Iif($msg['aid'] == 888888, "机器人(888888)", "未知客服($msg[aid])")),
			...
			if($totalpages > 1){
				TableRow(GetPageList(BURL('rating'), $totalpages, $page, 10, array('s'=>urlencode($search), 'g'=>$groupid, 't'=>$time, 'o'=>$order)));
			}
		...
		echo '<script type="text/javascript">
			$(function(){
				var url = "' . BURL("rating") . FormatUrlParam(array('p'=>$page, 's'=>urlencode($search), 'g'=>$groupid, 't'=>$time)) . '";
