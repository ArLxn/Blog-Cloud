...
class c_guests extends Admin{
	...
	public function updateguests(){
		...
		Success('guests?p=' . $page. FormatUrlParam(array('k'=>$letter, 's'=>urlencode($search), 'g'=>$groupid, 't'=>$time, 'o'=>$order)));
	}
	...
	public function index(){
		...
		TableRow(array('<a class="do-sort" for="gid">ID</a>', '姓名', '意向分', '语言', '<a class="do-sort" for="logins">登录</a>', '踢出', '最后服务', '<a class="do-sort" for="upload">上传授权</a>', '浏览器', '来自页面', 'Email', '<a class="do-sort" for="phone">电话</a>', '地址', '备注', '归属地 (IP)', '<a class="do-sort" for="last">最后登陆</a>', '<input type="checkbox" id="checkAll" for="deletegids[]"> <label for="checkAll">删除</label>'), 'tr0');
		...
		if($maxrows['value'] < 1){
			...
			if($totalpages > 1){
				TableRow(GetPageList(BURL('guests'), $totalpages, $page, 10, array('k'=>$letter, 's'=>urlencode($search), 'g'=>$groupid, 't'=>$time, 'o'=>$order)));
			}
		...
		echo '<script type="text/javascript">
			$(function(){
				var url = "' . BURL("guests") . FormatUrlParam(array('p'=>$page, 'k'=>$letter, 's'=>urlencode($search), 'g'=>$groupid, 't'=>$time)) . '";
