...
class c_users extends Admin{
	...
	public function updateusers(){
		...
		Success('users?p=' . $page. FormatUrlParam(array('s'=>urlencode($search), 't'=>$time, 'o'=>$order)));
	}
	...
	public function index(){
        ...
        switch($order)
        {
			...
			default:
				$orderby = " activated ASC, aid DESC ";			
				$order = "aid.down";
				break;
		}
		...
		TableRow('<center><form method="post" action="'.BURL('users').'" name="searchrobot" style="display:inline-block;"><label>关键字:</label>&nbsp;<input type="text" name="s" size="14" value="'.$search.'">&nbsp;&nbsp;&nbsp;<label>日期:</label>&nbsp;<input type="text" name="t" class="date-input" value="' . $time . '" size="8">&nbsp;&nbsp;&nbsp;&nbsp;<input type="submit" value="搜索客服" class="cancel"></form></center>');
		...
		if($totalpages > 1){
			TableRow(GetPageList(BURL('users'), $totalpages, $page, 10, array('s'=>urlencode($search), 't'=>$time, 'o'=>$order)));
		}
		...
		echo '<script type="text/javascript">
			$(function(){
				var url = "' . BURL("users") . FormatUrlParam(array('p'=>$page, 's'=>urlencode($search), 't'=>$time)) . '";
