...
class c_messages extends Admin{
	...
	public function updatemessages(){
		...
		Success('messages?p=' . $page. FormatUrlParam(array('s'=>urlencode($search), 'g'=>$groupid, 't'=>$time, 'o'=>$order)));
	}
	...
	public function index(){
		...
		echo '<tr class="tr0"><td class=td><a class="do-sort" for="mid">ID</a></td><td class=td><a class="do-sort" for="fromid">发送人</a></td><td class=td><a class="do-sort" for="toid">接收人</a></td><td class="td" width="50%">对话内容</td><td class=td><a class="do-sort" for="time">记录时间</a></td><td class="td last"><input type="checkbox" id="checkAll" for="deletemids[]"> <label for="checkAll">删除</label></td></tr>';
		...
		if($maxrows['value'] < 1){
			...
			if($totalpages > 1){
				TableRow(GetPageList(BURL('messages'), $totalpages, $page, 10, array('s'=>urlencode($search), 'g'=>$groupid, 't'=>$time, 'o'=>$order)));
			}
		...
		echo '<script type="text/javascript">
			$(function(){
				var url = "' . BURL("messages") . FormatUrlParam(array('p'=>$page, 's'=>urlencode($search), 'g'=>$groupid, 't'=>$time)) . '";
	...
	public function filter(){
		...
		TableRow('<center><form method="post" action="'.BURL('messages/filter').'" name="searchmessages" style="display:inline-block;*display:inline;"><label>日期:</label>&nbsp;<input type="text" name="t" class="date-input" value="' . $time . '" size="8">&nbsp;&nbsp;&nbsp;&nbsp;<input type="submit" value="搜索客人" class="cancel"></form></center>');
		...
		if($maxrows['value'] < 1){
			...
			if($totalpages > 1){
				TableRow(GetPageList(BURL('messages/filter'), $totalpages, $page, 10, array('t'=>$time, 'o'=>$order)));
			}
		...
		echo '<script type="text/javascript">
			$(function(){
				var url = "' . BURL("messages/filter") . FormatUrlParam(array('p'=>$page, 't'=>$time)) . '";
