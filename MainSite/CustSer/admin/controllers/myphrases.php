...
class c_myphrases extends Admin{
	...
	public function updatemyphrases(){
		...
		Success('myphrases?p=' . $page. FormatUrlParam(array('s'=>urlencode($search), 'g'=>$groupid, 'o'=>$order)));
	}
	...
	public function index(){
		...
		if($maxrows['value'] < 1){
			...
			if($totalpages > 1){
				TableRow(GetPageList(BURL('myphrases'), $totalpages, $page, 10, 's', urlencode($search), 'g', $groupid, 'o', $order));
			}
		...
		<script type="text/javascript">
			$(function(){
				var url = "' . BURL("myphrases") . FormatUrlParam(array('p'=>$page, 's'=>urlencode($search), 'g'=>$groupid)) . '";
