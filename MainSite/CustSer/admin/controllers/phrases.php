...
class c_phrases extends Admin{
	...
	public function updatephrases(){
		...
		Success('phrases?p=' . $page. FormatUrlParam(array('s'=>urlencode($search), 'g'=>$groupid, 'o'=>$order)));
	}
	...
	public function index(){
		...
		TableRow(array('<a class="do-sort" for="aid">所属客服</a>', '<a class="do-sort" for="sort">排序</a>', '<a class="do-sort" for="activated">状态</a>', '短语 (中)', '短语 (英)', '<input type="checkbox" id="checkAll" for="deletepids[]"> <label for="checkAll">删除</label>'), 'tr0');
		...
		if($maxrows['value'] < 1){
			...
			if($totalpages > 1){
				TableRow(GetPageList(BURL('phrases'), $totalpages, $page, 10, 's', urlencode($search), 'g', $groupid, 'o', $order));
			}
		...
		<script type="text/javascript">
			$(function(){
				var url = "' . BURL("phrases") . FormatUrlParam(array('p'=>$page, 's'=>urlencode($search), 'g'=>$groupid)) . '";
