...
$footer = '</div>
<div id="copyright">
	'.date("Y").' &copy; WeLive '. $WeLiveVersion .' <a href="http://www.weensoft.cn" target="_blank">为因软件 weensoft.cn</a>
</div>
</body>
</html>';
...
if(isset($_POST['install'])){
	...
	@chmod(ROOT . 'config/welive_questions.php', 0777);
	@chmod(ROOT . 'config/1f5fb85bd9c392d03f659a9c15756777.txt', 0777);
	@chmod(ROOT . 'upload/img/', 0777);
	@chmod(ROOT . 'upload/file/', 0777);
	...
	if(!is_writeable(ROOT . 'config/welive_questions.php')) {
		$installerrors[] = '请将系统配置文件config/welive_questions.php设置为可写, 即属性设置为: 777';
	}
	...
	if (!is_writable(ROOT . 'upload/file/')){
		$installerrors[] = '请将upload/file/文件夹的属性设置为: 777';
	}
